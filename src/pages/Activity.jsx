...
// ── Follow button ─────────────────────────────────────────────────────────────
// NOTE: follow state now lives in the parent (Activity) and is passed down.
// This is what keeps two notification cards for the SAME user in sync, and
// what stops duplicate follow requests from firing.
...
export default function Activity() {
  ...
  // ── Shared follow state, keyed by userId ───
  // This is the fix: a single source of truth per user, instead of each
  // FollowButton instance owning its own isolated `following` state.
  // If the same person shows up in two notification cards, both cards
  // read/write the same entry here, so they can never disagree, and a
  // second "Follow back" click for someone you already follow just calls
  // unfollow (or is a no-op) instead of firing a duplicate follow request.
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      ...
      // Seed followingMap from whatever the backend tells us about each
      // notif.user. Requires the API to include `isFollowing` per user
      // (see backend note below). Falls back to false if not provided.
  ...
  // ── Toggle follow, guarded against duplicate/in-flight requests ───
  const toggleFollow = useCallback(async (userId, currentlyFollowing) => {
    if (!userId) return;
    // Already have a request in flight for this user — ignore the click
    // instead of letting a second one queue up.
    ...
    try {
      ...
      // Update the ONE shared entry for this user — every card referencing
      // this userId re-renders with the new state automatically.

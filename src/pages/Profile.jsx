...
export default function Profile() {
  ...
  const handleFollow = async () => {
    ...
    try {
      ...
      // NOTE: getUserProfileById (controllers/userController.js), which
      // backs GET /api/users/id/:id, returns flat `followersCount` /
      // `followingCount` fields — NOT a nested `stats` object. That
      // mismatch (frontend was reading profileData.stats.followers,
      // which never existed on this response shape) was the root cause
      // of Followers/Following always showing 0. The optimistic update
      // below and the displayUser mapping further down both now read
      // the flat fields to match what the backend actually returns.
      setProfileData((prev) => ({
        ...prev,
      ...
      setToast({ message: t("toast.followFailed"), type: "error" });
      setTimeout(() => setToast(null), 2000);
    } finally {
  ...
  const handleLogout = () => {
    logout();
    setToast({ message: t("toast.signedOut"), type: "success" });
    setTimeout(() => {
      setToast(null);
      navigate("/login");
    }, 1500);
  ...
  //
  // NOTE on followers/following: GET /api/users/id/:id is backed by
  // getUserProfileById (controllers/userController.js), which returns
  // FLAT fields on the user object:
  //   { user: { followersCount, followingCount, ... } }
  // NOT nested under `stats` (that nested shape only exists on the
  // separate GET /api/users/:handle route in routes/users.js, which this
  // page does not call). Reading `profileData.stats.followers` here was
  // the root cause of Followers/Following always showing 0 — `stats`
  // never existed on this response, so the `|| "0"` fallback always won.
  // Fixed to read the flat `followersCount` / `followingCount` fields
  // that this endpoint actually returns.
  //
  // Note: name/handle/bio/focus fallback strings intentionally stay in
  // English literals below ("Your Name", "@yourname", etc.) — they only
  // ever show for brand-new/incomplete profiles before the user has set
  // real data, so translating them is optional. Move them into t() the
  // same way as everything else here if you want full coverage.
  const displayUser = {
  ...
  const achievements = useMemo(() => {
    ...
    if (stats.streak >= 7) {
      list.push({
        ...
        label: t("achievements.dayStreak", { count: stats.streak }),
    ...
    if (stats.workout > 0) {
      list.push({
        ...
        label: t("achievements.workoutsCount", { count: stats.workout }),
    ...
    if (stats.prs > 0) {
      list.push({
        ...
        label: t("achievements.prsCount", { count: stats.prs }),
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white pt-4 pb-28 px-4 max-w-lg mx-auto relative">
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold">{t("achievements.title")}</h2>
          </div>
      ...
      <div className="flex bg-[#13131f] rounded-2xl p-1 mb-4 border border-white/5">
        {[
          { key: "Post", label: t("tabs.post"), icon: <Grid size={14} /> },
          {
            key: "Stats",
            label: t("tabs.stats"),

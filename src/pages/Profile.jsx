...
export default function Profile() {
  ...
  const computedStreak = useMemo(() => computeStreak(posts, now), [posts, now]);
  ...
  // ── CHANGED: Qualitative consistency label — replaces the raw workout
  // count in the hero card so a novice user isn't confronted with "0".
  // Uses the same real data, just framed as a level instead of a number.
  const consistencyLabel =
    stats.workout > 0
      ? t("header.buildingConsistency")
      : t("header.justStarting");
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white pt-4 pb-24 px-4 max-w-lg mx-auto relative">
      ...
      <div className="bg-[#13131f] rounded-3xl p-5 mb-6 border border-white/5 relative overflow-hidden">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-white/5">
          {/* ── CHANGED: Consistency level (qualitative, not a raw count) ── */}

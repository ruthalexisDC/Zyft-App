...
// // NOTE: This is a large, freeform list of exercise names used both as
// // search suggestions AND as the actual value saved to the backend when
// // picked (or typed as a custom name). Translating display text here while
// // keeping a different value underneath would break what gets saved/shown
// // elsewhere (workout history, other users' feeds, etc). Left as English
// // data for now — worth a separate pass later if you want localized
// // exercise names stored distinctly from display labels.
// const EXERCISE_OPTIONS = [
//   "Bench Press",
//   "Incline Bench Press",
//   "Squat",
...
// // Internal data values for the category <select> — NOT translated directly.
// // Display option text comes from t(`logworkout:categoryLabels.${value}`).
...
//   // Tracks which exercise row's search dropdown is currently open
//   const [openSearchIndex, setOpenSearchIndex] = useState(null);
//   // Toast state for the success popup after posting a workout
...
//       if (res.ok) {
//         // Show success toast, then navigate home after a short delay
//         setToast({
//           type: "success",
//           message: t("logworkout:success.workoutLogged"),
//         });
//         setTimeout(() => navigate("/home"), 1800);
//       } else {
//         const err = await res.json();
//         console.error("🔍 Workout save error:", err);
//         alert(err.message || t("logworkout:errors.saveFailed"));
//       }
//     } catch (err) {
//       console.error("🔍 Network error:", err);
//       alert(t("logworkout:errors.networkError"));
//     } finally {
//       setLoading(false);
//     }
//   };
...
//       {/* Header */}
//       <div className="pt-6 pb-4">
//         <h1 className="text-xl font-bold">{t("logworkout:header")}</h1>
//         <p className="text-xs text-gray-500 mt-1">{t("logworkout:subtitle")}</p>
//       </div>
...
//             value={workoutData.title}
//             onChange={handleChange}
//             placeholder={t("logworkout:form.titlePlaceholder")}
//             className="w-full p-3 rounded-xl bg-[#13131f] border border-white/5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/50"
//             required
//           />
//         </div>
...
//         <div>
//           <label className="block text-xs text-gray-400 mb-1.5">
//             {t("logworkout:form.categoryLabel")}
//           </label>
//           <select
//             name="category"
//             value={workoutData.category}
...
//                 {t(`logworkout:categoryLabels.${val}`)}
//               </option>
//             ))}
//           </select>
//         </div>
...
//           <div>
//             <label className="block text-xs text-gray-400 mb-1.5">
//               {t("logworkout:form.durationLabel")}
//             </label>
//             <input
//               type="number"
//               name="duration"
//               value={workoutData.duration}
//               onChange={handleChange}
//               placeholder={t("logworkout:form.durationPlaceholder")}
//               className="w-full p-3 rounded-xl bg-[#13131f] border border-white/5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-purple-500/50"
//               required
//             />
//           </div>
//           <div>
//             <label className="block text-xs text-gray-400 mb-1.5">
//               {t("logworkout:form.caloriesLabel")}
//             </label>
//             <input
//               type="number"
//               name="caloriesBurned"
//               value={workoutData.caloriesBurned}
//               onChange={handleChange}
//               placeholder={t("logworkout:form.caloriesPlaceholder")}
//               className="w-full p-3 rounded-xl bg-[#13131f] border border-white/5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-purple-500/50"
//             />
//           </div>
//         </div>
...
//                           type="text"
//                           placeholder={t(
//                             "logworkout:exercises.searchPlaceholder",
//                           )}
//                           value={ex.name}
//                           onFocus={() => setOpenSearchIndex(i)}
//                           onBlur={() =>
...
//                         ex.name.trim() && (
//                           <div className="absolute z-10 left-0 right-0 mt-2 rounded-xl bg-[#1a1a26] border border-white/10 shadow-lg px-3 py-2 text-xs text-gray-500">
//                             {t("logworkout:exercises.noMatches", {
//                               name: ex.name,
//                             })}
//                           </div>
//                         )}
...
//                     <input
//                       type="number"
//                       placeholder={t("logworkout:exercises.sets")}
//                       value={ex.sets}
//                       onChange={(e) =>
//                         handleExerciseChange(i, "sets", e.target.value)
//                       }
...
//                     <input
//                       type="number"
//                       placeholder={t("logworkout:exercises.reps")}
//                       value={ex.reps}
//                       onChange={(e) =>
//                         handleExerciseChange(i, "reps", e.target.value)
//                       }
...
//                     <input
//                       type="number"
//                       placeholder={t("logworkout:exercises.weightUnit")}
//                       value={ex.weight}
//                       onChange={(e) =>
//                         handleExerciseChange(i, "weight", e.target.value)
//                       }
...
//             onChange={handleChange}
//             rows="3"
//             placeholder={t("logworkout:form.notesPlaceholder")}
//             className="w-full p-3 rounded-xl bg-[#13131f] border border-white/5 text-sm text-white placeholder-gray-600 resize-none focus:outline-none focus:border-purple-500/50"
//           />
//         </div>
...
//               <div className="flex-1">
//                 <p className="text-sm text-white">
//                   {file ? file.name : t("logworkout:photo.choosePhoto")}
//                 </p>
//                 <p className="text-[10px] text-gray-500">
//                   {file
//                     ? t("logworkout:photo.tapToChange")
//                     : t("logworkout:photo.optional")}
//                 </p>
//               </div>
//             </label>
//           </div>
...
//             <img
//               src={URL.createObjectURL(file)}
//               alt={t("logworkout:photo.previewAlt")}
//               className="w-full h-48 object-cover rounded-xl mt-2"
//             />
//           )}
//         </div>
...
//             <>
//               <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
//               {t("logworkout:submit.uploading")}
//             </>
//           ) : loading ? (
//             <>
//               <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
//               {t("logworkout:submit.saving")}
//             </>
//           ) : (
//             <>
//               <Dumbbell size={18} />
//               {t("logworkout:submit.logWorkout")}
//             </>
//           )}
//         </button>
//       </form>
...
export default function LogWorkout() {
  ...
  const [file, setFile] = useState(null);
  // Stable blob URL for the image preview — created/revoked via useEffect
  // so we never leak object URLs across renders.
  ...
  // Create a stable blob URL for the preview image and revoke the old one
  // whenever `file` changes. Without this, URL.createObjectURL() in the
  // render path creates a new URL on every re-render and leaks memory.
  useEffect(() => {
    if (!file) {
      setPreviewUrl(null);
      return;

  ...
  const handleFileChange = useCallback(
    async (e) => {
      ...
      try {
        ...
        setPreviewImage(data.avatar);
        // Pass a plain object — updateUser merges it with prev internally.
        updateUser({ avatar: data.avatar, photo: data.photo });
        showToast(t("toast.photoSuccess"));
      } catch (err) {
        console.error("Failed to upload photo:", err);
        showToast(t("toast.photoError"), "error");
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        // Pass a plain object — updateUser merges it with prev internally.
        // For avatar/photo, fall back to the current user values via user
        // (from useAuth) since we don't have access to prev here.
        updateUser({
          ...data.user,
          name: data.user.name,
          bio: data.user.bio,

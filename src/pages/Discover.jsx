...
// ─── Section skeletons that mirror each section's real layout ───
const TrendingSkeleton = () => (
  <div className="space-y-2">
    {Array.from({ length: TRENDING_PREVIEW_COUNT }).map((_, i) => (
      <div
        key={i}
        className="bg-[#13131f] rounded-2xl border border-white/5 border-l-4 border-l-white/10 flex items-center gap-4 px-4 py-3.5"
      >
...
const RecsSkeleton = () => (
  <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4 no-scrollbar">
    {Array.from({ length: 4 }).map((_, i) => (
      <div
        key={i}
        className="shrink-0 w-44 bg-[#13131f] rounded-2xl p-4 border border-white/5 flex flex-col gap-3"
      >
